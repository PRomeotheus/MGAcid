...
void GeState::handle_command(const GuestMemory &memory, std::uint32_t command, std::uint32_t data) {
    ...
    // reading off the game rather than recalling: MGA_TRACE_MATERIAL reports
    // every distinct value each of them is given.
    if (static const bool trace = std::getenv("MGA_TRACE_MATERIAL") != nullptr;
    ...
    case kTextureFunction:
        ...
        // Bit 16 tells the GE to double the result of the texture function,
        // clamped. The game's textures are authored for it, so dropping the
        // flag renders everything at half brightness.
        {
            static const bool no_doubling = std::getenv("MGA_NO_COLOR_DOUBLING") != nullptr;
            texture_.color_double = ((data >> 16u) & 1u) != 0u && !no_doubling;
        }
        break;
...
// MGA_TRACE_FB_TEXTURES also lists the commands the GE state ignores, with
...
void GeState::draw_primitive(const GuestMemory &memory, std::uint32_t data) {
    ...
    if (static const bool trace = std::getenv("MGA_TRACE_LIGHTING") != nullptr; trace && lighting_enabled_) {
