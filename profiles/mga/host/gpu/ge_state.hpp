...
struct TextureState {
    ...
    bool color_double{};             // TFUNC bit 16: GE doubles the texture result
...
// running game with MGA_TRACE_LIGHTING, not taken from a reference table.
struct LightState {
    bool enabled{};
...
// MGA_TRACE_FB_TEXTURES so a texture read from a copy can be traced back to
