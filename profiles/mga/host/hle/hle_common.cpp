...
bool HleRegistrar::try_add(std::string_view library, std::string_view name, HleFunction function) {
    ...
    // Time spent in here is the host's, not the game's: a kernel call that
    // waits for vsync or reads the disc costs the guest nothing it can see.
    // The kernel charges emulated time for stretches of translated code, so
    // tell it where each such stretch begins again.
    runtime_.register_hle(std::string(library), found->second,
                          [handler = std::move(function)](Runtime &rt, AllegrexContext &ctx) {
                              handler(rt, ctx);
                              kernel().guest_execution_resumes();
                          });
...
bool trace_sync() {
    static const bool enabled = std::getenv("MGA_TRACE_SYNC") != nullptr;
    return enabled;
}

void log_sync(const std::string &message) {
    static std::uint64_t remaining = [] {
        const char *limit = std::getenv("MGA_TRACE_SYNC_LIMIT");
