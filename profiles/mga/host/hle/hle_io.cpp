...
struct OpenFile {
    ...
    std::vector<std::string> listing;  // Directory: entry names sceIoDread returns
};
...
bool trace_io() {
    static const bool enabled = std::getenv("MGA_TRACE_IO") != nullptr;
    return enabled;
}
...
// Virtual time a UMD read of `bytes` takes: a fixed cost per request plus the
// transfer. MGA_UMD_KBPS sets the transfer rate (0 makes reads instant).
std::uint64_t umd_read_time_us(std::size_t bytes) {
    static const std::uint64_t kilobytes_per_second = [] {
        const char *text = std::getenv("MGA_UMD_KBPS");
        return text != nullptr ? std::strtoull(text, nullptr, 10) : 1800ull;
    }();
    if (kilobytes_per_second == 0u) return 0u;
    constexpr std::uint64_t kRequestUs = 200u;
    return kRequestUs + static_cast<std::uint64_t>(bytes) * 1000u / kilobytes_per_second;
}
...
void register_io(HleRegistrar &hle, const std::filesystem::path &disc_image, const std::filesystem::path &memory_stick) {
    ...
    hle.add("IoFileMgrForUser", "sceIoRead", [](Runtime &rt, AllegrexContext &ctx) {
        ...
        // A UMD read takes time on the PSP, and the game's threads rely on it:
        // Metal Gear Ac!d's sound thread only gets to a request the game queued
        // while the main thread waits for the disc. The reading thread blocks
        // for the time the drive would take, in virtual time.
        if (file.kind == OpenFile::Kind::Disc) {
            if (const std::uint64_t delay = umd_read_time_us(buffer.size()); delay != 0u) {
                kernel().delay_current(ctx, delay, static_cast<std::uint32_t>(buffer.size()));
                return;
            }
        }
    ...
    hle.add("IoFileMgrForUser", "sceIoDopen", [](Runtime &rt, AllegrexContext &ctx) {
        ...
        file.path = path;
        if (split.device == Device::Disc) {
            file.listing = io().disc->list(split.path);
        } else {
            std::error_code ec;
            for (const auto &entry : std::filesystem::directory_iterator(host_path(split.path), ec))
                file.listing.push_back(entry.path().filename().string());
    ...
    // sceIoDread(fd, SceIoDirent*): 1 while entries remain, 0 at the end. The
    // dirent is a SceIoStat (0x58 bytes) followed by the 256-byte name.
    hle.add("IoFileMgrForUser", "sceIoDread", [](Runtime &rt, AllegrexContext &ctx) {
        ...
        OpenFile &dir = found->second;
        if (dir.position >= dir.listing.size()) {
            kernel().finish(ctx, 0u);
            return;
        }
        const std::string name = dir.listing[static_cast<std::size_t>(dir.position++)];
        const std::uint32_t dirent = arg(ctx, 1);
        auto &memory = rt.memory();
        for (std::uint32_t i = 0; i < 0x58u; i += 4u) memory.store32(dirent + i, 0u);
        const SplitPath split = split_path(dir.path);
        const std::string child = split.path.empty() ? name : split.path + "/" + name;
        if (split.device == Device::Disc && io().disc) {
            if (const auto entry = io().disc->find(child))
                write_stat(memory, dirent, entry->directory, entry->size, entry->lba);
        } else {
            std::error_code ec;
            const std::filesystem::path host = host_path(child);
            const bool directory = std::filesystem::is_directory(host, ec);
            write_stat(memory, dirent, directory, directory ? 0u : std::filesystem::file_size(host, ec), 0u);
        }
        write_cstring(memory, dirent + 0x58u, name, 256u);
        if (trace_io()) std::cerr << "[io] dread " << dir.path << " -> " << name << "\n";
