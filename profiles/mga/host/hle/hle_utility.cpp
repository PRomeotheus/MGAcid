...
// (MGA_TRACE_OSK): the common dialog header, then at 0x30 the number of
...
// MGA_TRACE_OSK: every call of the keyboard utility, with the words of
// its parameter block and of the first field, so their layout is read off the
// game rather than recalled.
bool trace_osk() {
    static const bool trace = std::getenv("MGA_TRACE_OSK") != nullptr;
    return trace;
}
