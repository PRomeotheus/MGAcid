...
public:
    ...
    void start() noexcept {
        status_ = dialog_status::kInit;
        held_ = false;
    }

    // Keeps the dialog at VISIBLE until release(), for one the player answers
    // on screen rather than one the host settles in InitStart. The guest goes
    // on polling and presenting frames meanwhile, which is what draws it.
    // Whoever holds a dialog must release it on every path out, or the guest
    // waits for good.
    ...
    std::uint32_t poll() noexcept {
        ...
        else if (status_ == dialog_status::kVisible && !held_) status_ = dialog_status::kQuit;
