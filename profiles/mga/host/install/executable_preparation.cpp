...
std::vector<std::uint8_t> prepare_executable(std::span<const std::uint8_t> eboot_bin,
    ...
    // A plain ELF (Metal Gear Ac!d's SYSDIR/BOOT.BIN) needs no decryption; the
    // hash above already pins it.
    if (eboot_bin.size() >= 4u && std::memcmp(eboot_bin.data(), "\x7F" "ELF", 4u) == 0) {
