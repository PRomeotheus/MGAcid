...
constexpr const char *kOrganization = "MGAcid";
constexpr const char *kApplication = "MGA";
...
std::filesystem::path user_data_directory() {
    if (const char *dir = std::getenv("MGA_DATA_DIR"); dir != nullptr && *dir != '\0') return path_from_utf8(dir);
#if defined(MGA_HAS_SDL)
...
#endif
    if (base.empty()) throw psprecomp::Error("Cannot determine the user data directory; set MGA_DATA_DIR");
