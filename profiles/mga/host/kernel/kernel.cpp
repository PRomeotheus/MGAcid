...
// Guest time for a thread that makes no kernel call, charged when the runtime
// preempts it -- every kStarvationInterval dispatches of translated code.
//
// What this has to get right is not the game's speed but how emulated time
// passes *inside* a burst of guest code. The game's own vblank waits move the
// clock a frame at a time and pace_to_real_time() sleeps off the surplus, so
// the speed comes out right either way. But Metal Gear Ac!d's sound driver
// runs four threads that pace themselves on sceKernelDelayThread with delays
// of one to three milliseconds, and they have to get to a queued request
// before the thread that posted it moves on. On hardware they do, because the
// posting thread's burst of work takes milliseconds of the same clock. If a
// burst costs the guest clock almost nothing, they never wake inside one, and
// the file thread reads a request that is not filled in yet -- the crash at
// "Checking Memory Stick Duo".
//
// So time is charged for the host time spent running translated code, which is
// what work costs. Two things it is not: host time spent blocked inside a
// kernel call the guest made (hle_common.cpp resets the measurement when one
// returns, or the ~15 ms the Vulkan present waits for vsync would arrive as
// one lump with the work either side of it free), and a count of dispatches,
// which sounds machine-independent but is not work: a generated unit runs
// whole loops without returning to the dispatcher, so one dispatch can be ten
// instructions or ten thousand.
//
// That leaves the ratio between host time and PSP time, which is a property of
// this machine and this build. Rather than a constant that is wrong everywhere
// but one PC, it is measured: each frame, the scale is nudged towards the
// value that would have charged kTargetWorkUsPerFrame for the work that frame
// actually took. A slow machine settles on a small scale and the game runs at
// the right speed with fewer frames in it, exactly as an emulator should
// degrade; a fast one settles high and has headroom to spare. MGA_CPU_SCALE
// pins it to a fixed value instead.
//
// The target is what a PSP game does with a frame: most of it computing, the
// rest waiting for the vblank.
constexpr std::uint64_t kTargetWorkUsPerFrame = 12'000u;
constexpr std::uint64_t kMinCpuScale = 1u;
constexpr std::uint64_t kMaxCpuScale = 100'000u;
// A boundary may not charge more than this, whatever it is asked for: only a
// host pathology (descheduled, a driver stall, a breakpoint) gets near it.
constexpr std::uint64_t kDefaultStarvationQuantumUs = 4000u;
...
bool trace_enabled() {
    static const bool enabled = std::getenv("MGA_TRACE_KERNEL") != nullptr;
    return enabled;
}
...
void Kernel::install(Runtime &runtime, std::uint32_t gp, std::uint32_t image_end) {
    ...
    runtime.register_function(kThreadExitStub, &native_thread_exit, "mga_thread_exit");
    runtime.register_function(kInterruptReturnStub, &native_interrupt_return, "mga_interrupt_return");
    runtime.register_function(kIdleStub, &native_idle, "mga_idle");
    runtime.register_function(kGuestCallReturnStub, &native_guest_call_return, "mga_guest_call_return");
    // How often the runtime preempts a thread that makes no kernel call, in
    // dispatches. MHP3rd runs well at 20000; Metal Gear Ac!d's loader
    // decompresses whole archives without a kernel call, and its sound thread
    // (woken from an interrupt) must get to requests before the main thread
    // reuses their buffers, so it needs the finer grain.
    std::uint64_t interval = 200u;
    if (const char *text = std::getenv("MGA_STARVATION_INTERVAL")) interval = std::strtoull(text, nullptr, 0);
    starvation_quantum_us_ = kDefaultStarvationQuantumUs;
    if (const char *text = std::getenv("MGA_STARVATION_QUANTUM_US"))
        starvation_quantum_us_ = std::max<std::uint64_t>(1u, std::strtoull(text, nullptr, 0));
    // Without a fixed scale, calibrate_cpu_scale() maintains it per frame.
    if (const char *text = std::getenv("MGA_CPU_SCALE")) {
        cpu_scale_ = std::clamp<std::uint64_t>(std::strtoull(text, nullptr, 0), kMinCpuScale, kMaxCpuScale);
        cpu_scale_fixed_ = true;
    }
...
void Kernel::finish(AllegrexContext &ctx, std::uint32_t result) {
    ...
    // Interrupts suspended means no preemption: on hardware the scheduler runs
    // off the timer interrupt, so sceKernelCpuSuspendIntr is how the game makes
    // a critical section. Switching threads here anyway is what let the sound
    // driver's request queue be read while it was half updated -- a lost
    // request that hangs the game, or a half-written one that crashes it.
    if (!dispatch_enabled_ || !interrupts_enabled_) return;
...
void Kernel::schedule(AllegrexContext &ctx) {
    for (;;) {
        ...
        // Every thread is waiting. That is normal between frames and is the
...
// Whether emulated time follows the host clock. Runs without a window, and the
...
bool Kernel::pace_to_real_time() {
    ...
    // three times PSP speed on a 60-90 Hz display.
    if (!real_time_clock_active()) {
...
std::uint64_t Kernel::guest_time_for_uninterrupted_work() {
    using Clock = std::chrono::steady_clock;
    const Clock::time_point now = Clock::now();
    if (last_work_boundary_ == Clock::time_point{}) {
        last_work_boundary_ = now;
        return 0u;
    }
    // Whole microseconds only, with the remainder left on the clock: boundaries
    // can be closer together than a microsecond, and rounding each one down
    // would stop time a nanosecond at a time.
    const auto elapsed_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(now - last_work_boundary_).count();
    const auto elapsed_us = static_cast<std::uint64_t>(elapsed_ns / 1000);
    last_work_boundary_ += std::chrono::microseconds(elapsed_us);
    frame_work_us_ += elapsed_us;
    std::uint64_t charge = elapsed_us * cpu_scale_;
    if (charge > starvation_quantum_us_) {
        charge = starvation_quantum_us_;
        last_work_boundary_ = now;
    }
    return charge;
}

// Called once per presented frame. Moves the scale towards the value that
// would have charged a frame's worth of guest time for this frame's work,
// slowly enough that one heavy frame does not swing it.
void Kernel::calibrate_cpu_scale() {
...
// A frame is the sign of life this watches for: the game can be busy without
// drawing for a moment, but not for fifteen seconds.
void Kernel::check_for_hang(const AllegrexContext &ctx) {
    using Clock = std::chrono::steady_clock;
    static const std::uint64_t limit_s = [] {
        const char *text = std::getenv("MGA_HANG_REPORT_S");
        return text != nullptr ? std::strtoull(text, nullptr, 0) : 10ull;
    }();
    if (limit_s == 0u) return;
    const Clock::time_point now = Clock::now();
    if (last_frame_ == Clock::time_point{}) {
        last_frame_ = now;
        return;
    }
    const auto quiet_s = std::chrono::duration_cast<std::chrono::seconds>(now - last_frame_).count();
    // Report at the limit and then ever less often, so a sampled pc shows
    // whether the thread is going round a loop or has stopped somewhere.
    if (quiet_s < static_cast<std::int64_t>(limit_s * (hang_reports_ + 1u))) return;
    if (hang_reports_ >= 4u) return;
    ++hang_reports_;
    Thread *thread = current_thread();
    std::cerr << "[hang] no frame for " << quiet_s << " s, running "
              << (thread != nullptr ? thread->name : std::string("?")) << " at " << psprecomp::hex32(ctx.pc)
              << " (ra=" << psprecomp::hex32(ctx.gpr[31]) << " sp=" << psprecomp::hex32(ctx.gpr[29])
              << " gp=" << psprecomp::hex32(ctx.gpr[28]) << ")\n"
              << describe_threads() << std::endl;
}
...
void Kernel::on_starvation(AllegrexContext &ctx) {
    ...
    // Same here: a preemption boundary is the timer interrupt, and the guest
    // has asked for it to be held off.
    if (!dispatch_enabled_ || !interrupts_enabled_) return;
    Thread *best = best_ready_thread();
