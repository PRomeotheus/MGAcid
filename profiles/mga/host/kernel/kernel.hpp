...
inline constexpr std::uint32_t kUserMemoryEnd = 0x0A000000u;  // 32 MiB PSP-1000 layout
...
public:
    ...
    // Forgets how far emulated time was ahead of or behind real time, and how
    // long ago the running thread was last preempted. Called after the game was
    // paused, so it resumes at normal speed instead of racing to make up the
    // pause -- and so the pause itself is not charged to the game's clock.
    void resync_real_time() noexcept {
        pacing_started_ = false;
        last_work_boundary_ = {};
    }
    // A kernel call has returned and translated code runs again: only the time
    // from here to the next preemption boundary is the game working.
    void guest_execution_resumes() noexcept { last_work_boundary_ = std::chrono::steady_clock::now(); }
    // A frame has been presented; re-measures how fast this machine runs guest
    // code, and tells the hang watchdog the game is still drawing.
    void calibrate_cpu_scale();
    // MGA_HANG_REPORT_S (default 15, 0 disables): when no frame has been
    // presented for that long, say what every thread is doing and where the
    // running one is, a few times over, so a spin shows up as a pc that does
    ...
    [[nodiscard]] std::uint64_t guest_time_for_uninterrupted_work();
...
private:
    ...
    // Guest time charged at each preemption boundary; see kernel.cpp.
    std::uint64_t starvation_quantum_us_{4000u};
    std::uint64_t cpu_scale_{40u};
