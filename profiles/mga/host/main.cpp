...
void ensure_main_stack(char **argv) {
    ...
    constexpr const char *kMarker = "MGA_STACK_RAISED";
    rlimit limit{};
    ...
    // the process to "exe".
    const std::filesystem::path self = mga::executable_path();
...
Options parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (options.install && !options.install_image) options.install_image = mga::install::path_from_utf8(arg);
...
// The layout a game_dir has always had; profiles/mga/game by default.
GameFiles files_in_game_directory(const std::filesystem::path &game_dir) {
    ...
    files.memory_stick = mga::memory_stick_directory(game_dir);
...
// profiles/mga/game in the checkout this was built from, for developer
// builds; empty in a release build, which must not depend on the machine it
// was built on.
std::filesystem::path checkout_game_directory() {
#if defined(MGA_DEFAULT_GAME_DIR)
    return MGA_DEFAULT_GAME_DIR;
#else
    return {};
#endif
}
...
// installer fills, then profiles/mga/game in a developer build. With none of
...
std::optional<GameFiles> locate_game(const Options &options) {
    namespace install = mga::install;
    if (options.game_dir) return files_in_game_directory(*options.game_dir);
    if (const char *dir = std::getenv("MGA_GAME_DIR"); dir != nullptr && *dir != '\0')
...
// SIGQUIT (kill -QUIT, or Ctrl-\\ in a terminal) writes the modules that are
// loaded right now to MGA_DUMP_MODULES and leaves. A stage corpus can only be
// generated from a stage as the game linked it in memory, and a stage the game
// never unloads -- the one it is hung in, say -- is otherwise unreachable.
// Windows has no SIGQUIT, so there it is SIGBREAK (Ctrl-Break) instead.
#if defined(SIGQUIT)
constexpr int kDumpSignal = SIGQUIT;
#elif defined(SIGBREAK)
constexpr int kDumpSignal = SIGBREAK;
#else
constexpr int kDumpSignal = 0;
#endif

psprecomp::Runtime *g_running_runtime = nullptr;

extern "C" void on_dump_signal(int) {
    if (g_running_runtime != nullptr) mga::dump_loaded_modules(*g_running_runtime);
    std::cout.flush();
    std::_Exit(0);
}
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        mga::ProfilePaths paths;
        paths.disc_image = files->disc_image;
        paths.memory_stick = files->memory_stick;
        if (!std::filesystem::is_regular_file(executable))
            throw psprecomp::Error("Missing " + executable.string() + " (run profiles/mga/scripts/prepare_game.sh)");

        const std::string sha256 = psprecomp::sha256_file(executable);
        if (sha256 != mga::install::kExecutableSha256)
        ...
        if (elf.required_ram_size(mga::kLoadBase) > mga::kGuestRamBytes)
            throw psprecomp::Error("Executable does not fit the 32 MiB PSP memory layout");

        psprecomp::Runtime runtime(mga::kGuestRamBytes);
        for (const EmbeddedNid &entry : kEmbeddedNids) runtime.nids().add(entry.library, entry.nid, entry.name);
        (void)elf.load_and_relocate(runtime.memory(), mga::kLoadBase);
        psprecomp::register_generated_functions(runtime);
        mga::install_profile(runtime, elf, paths);
        mga::install_call_watches(runtime);
        ...
        std::cout << "Yakumo PSP bootstrap\n"
                  ...
                  << "Entry:      " << psprecomp::hex32(elf.runtime_entry(mga::kLoadBase)) << "\n"
                  << "Functions:  " << runtime.function_count() << "\n";
        if (runtime.function_count() == 0u) {
            std::cout << "No generated functions are linked. Run profiles/mga/scripts/generate.sh and rebuild.\n";
            return 3;
        }

        try {
            runtime.run(elf.runtime_entry(mga::kLoadBase), configured_max_dispatches());
        } catch (const std::exception &) {
            // Return addresses still on the faulting thread's stack: a rough
            // guest backtrace, innermost first.
            const auto &cpu = runtime.cpu();
            std::cerr << "guest stack from sp=" << psprecomp::hex32(cpu.gpr[29]) << " (code addresses):";
            int shown = 0;
            for (std::uint32_t offset = 0; offset < 0x800u && shown < 16; offset += 4u) {
                const std::uint32_t address = cpu.gpr[29] + offset;
                if (!runtime.memory().contains(address, 4u)) break;
                const std::uint32_t word = runtime.memory().load32(address);
                if (word >= mga::kLoadBase && word < 0x0A000000u && (word & 3u) == 0u) {
                    std::cerr << " " << psprecomp::hex32(word) << "@+" << std::hex << offset << std::dec;
                    ++shown;
                }
            }
            std::cerr << "\n";
            // What every thread was doing: a crash in one of the game's own
            // threads is usually about what the others were waiting for.
            std::cerr << mga::kernel().describe_threads() << "\n";
            // The module that was running is usually the interesting one, and a
            // crash is the likeliest way a session ends while a stage is loaded.
            mga::dump_loaded_modules(runtime);
            // MGA_DUMP_ON_ERROR=addr:bytes[,addr:bytes...] dumps guest memory ("sp" names the stack).
            if (const char *spec = std::getenv("MGA_DUMP_ON_ERROR"); spec != nullptr) {
                ...
                while (start < list.size()) {
                    ...
                    const std::string where = item.substr(0, colon);
                    std::uint32_t address = where == "sp" ? cpu.gpr[29]
                                                          : static_cast<std::uint32_t>(std::stoul(where, nullptr, 16));
                    const std::uint32_t bytes = static_cast<std::uint32_t>(std::stoul(item.substr(colon + 1u), nullptr, 0));
                    for (std::uint32_t offset = 0; offset < bytes; offset += 16u) {
                        std::cerr << psprecomp::hex32(address + offset) << ":";
                        for (std::uint32_t w = 0; w < 16u; w += 4u) {
                            const std::uint32_t a = address + offset + w;
                            std::cerr << " " << (runtime.memory().contains(a, 4u) ? psprecomp::hex32(runtime.memory().load32(a)) : std::string("--------"));
                        }
                        std::cerr << "\n";
                    }
                }
        ...
        if (mga::install::setup_requested_on_exit()) return mga::install::restart_for_setup(argv[0]);
        // "Restart now" after importing a save.
        if (mga::install::restart_requested_on_exit()) return mga::install::restart(argv);
        std::cout << mga::kernel().describe_threads() << "\n";
