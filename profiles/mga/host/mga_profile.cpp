...
void install_profile(Runtime &runtime, const psprecomp::Elf32Image &elf, const ProfilePaths &paths) {
    ...
    const bool strict = std::getenv("MGA_STRICT_HLE") != nullptr;
    std::size_t stubbed = 0u;
    auto bound = hle.bound_imports();
    ...
    for (const auto &import : imports) {
        ...
        bound.insert({import.library, import.nid});
        ++stubbed;
    }
    // Modules the game loads later link against the same set; their exports
