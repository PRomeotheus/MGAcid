...
struct OverlayCorpus {
    ...
    std::uint64_t hash{};
    decltype(&mga_register_overlay) install{};
};
...
std::filesystem::path overlay_directory() {
    if (const char *dir = std::getenv("MGA_OVERLAY_DIR"); dir != nullptr && *dir != '\0') return dir;
...
void load_overlay_library(const std::filesystem::path &path) {
    ...
    const auto info_of = reinterpret_cast<decltype(&mga_overlay_info)>(
        library_symbol(handle, "mga_overlay_info"));
    const auto install = reinterpret_cast<decltype(&mga_register_overlay)>(
        library_symbol(handle, "mga_register_overlay"));
...
void dump_slot(const psprecomp::GuestMemory &memory, std::uint32_t slot_start, std::uint32_t slot_end) {
    const char *directory = std::getenv("MGA_DUMP_OVERLAYS");
    if (directory == nullptr) {
        log_once("overlay-dump-hint",
                 "[overlay] set MGA_DUMP_OVERLAYS=<dir> to dump the loaded overlay for recompilation");
        return;
    }
    ...
    std::cout << "[overlay] dumped " << image.size() / 1024u << " KiB to " << path.string() << "\n"
              << "[overlay] recompile it with: profiles/mga/tools/add_overlay.py <build_dir> "
