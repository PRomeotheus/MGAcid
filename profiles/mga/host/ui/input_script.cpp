...
struct State {
    ...
    // MGA_INPUT_LIVE: a file whose appended lines are read as they come.
...
void run(const Step &step) {
    ...
    if (step.action == "key") {
        ...
        const char *dir = std::getenv("MGA_SCREENSHOT_DIR");
...
void attach() {
    ...
    s.attached = true;
    bool uses_pad = false;
    if (const char *live = std::getenv("MGA_INPUT_LIVE"); live != nullptr && *live != '\0') {
        s.live_path = live;
    ...
    if (const char *text = std::getenv("MGA_INPUT_SCRIPT"); text != nullptr) {
