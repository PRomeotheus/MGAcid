...
std::string file_url(const std::string &path) {
    // A Windows path arrives with backslashes and a drive letter. A file URL
    // needs forward slashes, the colon left alone and a third slash before the
    // drive, or SDL_OpenURL refuses it ("Couldn't open given URL").
    std::string url = "file://";
    if (!path.empty() && path.front() != '/') url += '/';
    for (const unsigned char raw : path) {
        const unsigned char c = raw == '\\' ? '/' : raw;
        if (std::isalnum(c) != 0 || c == '/' || c == '-' || c == '_' || c == '.' || c == '~' || c == ':') {
...
void Menu::video() {
    ...
    }
    {
        ...
        if (toggle_row("Smooth distant textures", s.smooth_textures && !s.sharp_textures, o)) {
            s.smooth_textures = !s.smooth_textures;
...
void Menu::audio() {
    ...
    const auto locked = [&](const char *key, const char *description) {
        ...
        if (!device) {
            options.disabled = true;
            options.note = std::getenv("MGA_NO_AUDIO") != nullptr ? "Off: MGA_NO_AUDIO" : "No audio device";
        }
        return options;
    };
    section("Output");
...
// The on-screen network overlay (menu: Network, or MGA_ADHOC_OVERLAY).
bool &network_overlay() {
    static bool shown = [] {
        const char *text = std::getenv("MGA_ADHOC_OVERLAY");
...
void Menu::network() {
    ...
    if (toggle_row("Log every call and packet", adhoc::Client::tracing(),
                   {false, {}, "The same as MGA_TRACE_ADHOC=1: every ad hoc call and packet header goes to the "
