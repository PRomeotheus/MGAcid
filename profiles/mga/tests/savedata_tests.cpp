...
//
//   mga_savedata_tests
//   mga_savedata_tests --check <save folder> <data file name> <game key, 32 hex digits> [plaintext out]
//
...
void test_store() {
    const auto root = std::filesystem::temp_directory_path() / "mga_savedata_tests";
...
void test_transfer() {
    ...
    const auto root = fs::temp_directory_path() / "mga_save_transfer_tests";
    fs::remove_all(root);
    ...
    {
        std::fstream data(damaged / "MGA.BIN", std::ios::binary | std::ios::in | std::ios::out);
        data.seekp(200);
        data.put('\x7f');
    }
    ...
    fs::copy_file(source_saves / "ULJM05800" / "MGA.BIN", damaged / "MGA.BIN",
    ...
    const auto old_bytes = file_bytes(dest_saves / "ULJM05800" / "MGA.BIN");
    ...
    check(file_bytes(second.backup / "MGA.BIN") == old_bytes && fs::exists(second.backup / "PARAM.SFO"),
    ...
    check(timed_result.ok && file_bytes(timed / "ULJM05800" / "MGA.BIN") ==
                                 file_bytes(dest_saves / "ULJM05800" / "MGA.BIN"),
    ...
    const auto earlier = file_bytes(backups / "ULJM05800" / "MGA.BIN");
    check(!back_up_saves(dest, backups, false).ok && file_bytes(backups / "ULJM05800" / "MGA.BIN") == earlier,
          "an earlier backup is not replaced without asking");
    check(back_up_saves(dest, backups, true).ok && file_bytes(backups / "ULJM05800" / "MGA.BIN") ==
                                                       file_bytes(dest_saves / "ULJM05800" / "MGA.BIN"),
