...
def main(argv):
    ...
    with open(argv[1], "rb") as image:
        ...
        def extract(path, lba, size):
            # Compared without the leading '/': Git Bash rewrites an argument such as
            # /PSP_GAME/... into a Windows path, so callers pass PSP_GAME/... instead.
            if prefixes and not any(path.lstrip("/").startswith(prefix.lstrip("/")) for prefix in prefixes):
                return
